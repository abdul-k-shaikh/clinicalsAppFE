...
function CollectClinicals(){
    ...
    useEffect(()=>{
        axios.get('http://localhost:8080/api/patients/'+patientId).then(res=>{
    ...
    const handleSubmit=(event)=>{
     ...
     axios.post('http://localhost:8080/api/clinicals/', data).then(res=>res=>{
        toast("Patients Data added successfully")
     })
